...
if [[ $- == *i* && -r "$HOME/.bashrc" ]]; then
  ...
  echo "[~/.bash_profile] Either the shell is not running interactively, .bashrc doesn't exist, or the current user does not have read permissions on the file. Attempted to execute from ~/.bash_profile."
fi
